...
from .pivot_opt import refine_candidates
from .residual import build_tucker_cross_matrix
...
def _tucker_select_pivots(
    ...
    Start with rank 0 (U empty).  Loop up to ``target_rank`` times:
      ...
      2. If ``opts.gd_steps > 0``, locally refine each candidate's
         continuous ``z`` toward a nearby maximizer of ``e_k(alpha,·)^2``
         via ``pivot_opt.refine_candidates`` (a few projected Adam/GD
         steps; ``alpha`` is discrete and is never moved). Otherwise this
         is a no-op and behaves exactly as before: the sampled candidates
         are scored as-is.
      3. Accept the (possibly refined) candidate with the largest
         |residual|, provided it passes the absolute tolerance and
         conditioning guards.
      4. Rebuild S and U = S^{-1} with the new pivot included.

    ``opts.gd_steps == 0`` (the default) reproduces the original
    sample-and-argmax behaviour exactly: the pure-sampling arg-max was only
    ever as good as one batch of ``opts.n_starts`` candidates, with no local
    refinement around the sampled maximizer even though ``types.CandidateBatch``
    has fields reserved for exactly that (see the README's discussion of this
    gap). Setting ``opts.gd_steps > 0`` fills that gap.
    ...
    for _ in range(target_rank):
        ...
        z0_flat = jnp.asarray(z_candidates, dtype=jnp.float64)
        alpha_flat = jnp.asarray(alpha_ids, dtype=jnp.int32)

        # Local refinement: nudges each candidate's continuous z toward a
        # nearby maximizer of e_k(alpha,·)^2 before the arg-max below (a
        # no-op when opts.gd_steps == 0, in which case z_flat == z0_flat and
        # resid_vals is exactly the old sample-and-score result — same
        # rank-0-vs-residual branch as before, now inside refine_candidates).
        z_flat, resid_vals = refine_candidates(
            F,
            stage_ctx,
            alpha_flat,
            z0_flat,
